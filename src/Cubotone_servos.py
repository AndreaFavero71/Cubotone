...
def cube_scrambler(robot_moves='a'):
    ...
    for move in robot_moves.values():                           # iterates over the dictionary values (the keys is the amount of Kociemba moves)
        ...
        if spins!=0:
            spin(spins,s_debug)                          # servo package is called for the required spins
#             time.sleep(0.05)
        if flips!=0:
            flip(flips, s_debug, 'open', True)           # servo package is called for the required flips
#             time.sleep(0.05)
        if rotations !=0:
            rotate(rotations, s_debug)                   # servo package is called for the required rotations (lower cube's layer)
#             time.sleep(0.05)
        i+=1
