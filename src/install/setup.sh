...
if ! crontab -l 2>/dev/null | grep -q "Cubotone_bash.sh"; then
    (crontab -l 2>/dev/null; echo 'MAILTO=""'; echo @reboot su - pi -c \"/usr/bin/vncserver :0 -geometry 1280x720\"; echo '#@reboot /bin/sleep 5; bash -l /home/pi/cubotone/src/Cubotone_bash.sh > /home/pi/cubotone/src/Cubotone_terminal.log 2>&1') | crontab -
fi
